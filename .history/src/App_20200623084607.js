...
import {sha256} from './untils/SHA256'
...
export class App extends Component {
  ...
  constructor(props) {
    ...
    this.AmqpReturn = this.AmqpReturn.bind(this);
  ...
  AmqpReturn(frame) {
    ...
    let {OnlineCount} = this.state;
    var obj = JSON.parse(frame.body);
    
    if (obj.OnlineUser != null && obj.OnlineCount>= OnlineCount  ) {
      this.setState({OnlineCount :obj.OnlineCount});
      console.log("updata");
    }else
  ...
  test() {
    ...
    let {OnlineCount,Subscribe} = this.state;
    const { cookies } = this.props;
    var loginName = cookies.get('loginState');
    // var obj = { OnlineUser: loginName, OnlineCount: OnlineCount };
    // var data = JSON.stringify(obj);
    // client.send('/exchange/test/test',  this.AmqpReturn, data);
    if (client.connected == true && Subscribe == false) {
    ...
    var obj = { OnlineUser: loginName, OnlineCount: OnlineCount };
    var data = JSON.stringify(obj);
    client.send('/exchange/amq.fanout/test',{}, data);
    //client.send('/exchange/test/liuzh', {}, "Hello");
  }
  test2() {
    var obj = { OnlineUser: null, OnlineCount: 0 , data :'Hello'};
    var data = JSON.stringify(obj);
    client.send('/exchange/amq.fanout/test', {}, data);
  }
  render() {
            ...
            <Layout style={{ padding: '0 24px 24px' }}>
              ...
              >
                <Switch>
                  ...
                  <Route path="/jobpool">
                    <h2>Job Pool Online: {OnlineCount}</h2>
                    <CommandJobpool />
                  </Route>
